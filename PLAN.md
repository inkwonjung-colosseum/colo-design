...
- 2행과 4행이 함께 맞으면(병합 충돌 중인데 HEAD 가 딴 브랜치) 4행이 앞선다. 차선이 이 상태를 만들지 않으므로 이론상의 갈림길이다.
- 도구의 병합 흔적(`pendingOp`)이 남았는데 git 은 이미 끝난 상태는 어느 행도 잡지 않는다. 마무리한 실행부가 흔적을 지우고, 감독자가 틱마다 흔적과 git 상태를 맞춘다(단계 2c).
...
**어휘 통일.** 사용자에게 보이는 문자열의 `저장` · `넘기기` · `넘긴` · `넘김` ·
