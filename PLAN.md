...
| # | 관찰 | 조치 | 누가 | 턴 중 | 예산 키 |
| --- | --- | --- | --- | --- | --- |
| C | 클론이 손상됨 — HEAD · 인덱스를 읽는 탐침이 `index file corrupt` · `not a git repository` · `bad object` 를 말함, fsck 실패, 도구의 보관이 없는 객체로 트리를 짓지 못함 | 재클론: 구해 두기(`salvage/<시각>`) → 미리보기 멈춤 · 옛 클론 옮기기(`repo.corrupt-<시각>`) → 새로 받기 → 되살리기. 구해 두기 · 옮기기가 실패하면 개발자 알림 | 도구 | 아니요 | `reclone`(하루 1) |
...
| 14b | 반려 이유 반영 턴이 원장에 남음(`reviews[pr].pendingRejection` — 랜딩이 적고 떠난다) | 반영 턴을 보내고 기록을 지운다. 예산이 다하면 알림 한 번 | AI | 아니요 | `review:<pr>` |
...
**표를 구현하며 정한 것 (단계 2a).**
- C행(클론 손상, 단계 9)은 무결성 행(0~4)보다 앞선다 — 손상된 클론의 git 읽기는 믿을 수 없고, 그 위의 조치(abort · checkout · 보관)는 손상을 넓힐 뿐이다. 턴이 돌면 예 행도 보지 않는다. 절차는 원장 `reclone` 에 걸음마다 적혀 어디서 끊겨도 다음 틱이 잇고, 예산은 절차를 시작할 때 한 번만 쓴다.
...
| 5 | 합침 | 초대 v4 |
| 6 | 합침 | 제출 네 단계 · head 로 입양 · 도구 구간 · 캡처 전용 브랜치 · 채팅으로 제출. 웹의 제출 버튼을 `repo.submit` 에 잇는 일은 단계 10 으로 — 사용자 세션이 같은 자리를 고치는 중이었다 |
| 7 | 합침 | 코멘트 끝까지 읽기 · 봇 거르기 · 자동 답장 · 반려 반영(14b). 뒤따른 수정: 반려 표식이 주의를 영구히 세우던 결함, 자동 대화가 Claude 만 띄우던 결함(이제 쓸 수 있는 공급자로) |
| 8 | 합침 | 5단 재시도 · 로그인 만료 뒤 한 번 다시 · 조용한 크래시 · 꺼질 때 돌던 턴 이어받기 · 버튼 없는 잃은 말 · /compact 뒤 다시 |
| 9 | 합침 | 반려 브랜치 정리 · gc · fsck · 임시 폴더 · 캡처 크기 · 클론 손상 재클론(C행) · 설치 판정 · 저장소 이동 · 디스크 |
| 10 | 합침 | 제출 버튼 = `repo.submit` 한 번 · 실패 배너 · 원문 오류 · `상태 확인` · 개발자 코멘트 창 · `개발자 부르기` · 토큰 만료 카드를 걷고 주의 한 줄로 · 저장소 이름 · 요청 번호 · 파일 경로 · 컴포넌트 이름은 개발 실행에서만 · 어휘 60여 문장 · README. 사용자 세션의 화면 결정(제출 진행 표시 · 턴 뒤 자동 이동 · 주소창 목록 · `보기` 메뉴)은 그대로 |

통합은 `lifecycle-int` 브랜치에서 했다 — `lifecycle` 의 작업 트리에 사용자의 커밋되지 않은 화면 변경(`styles.css` 등)이 있어 그 트리에서는 병합이 거절됐다. 그 변경을 커밋(a74bb9d9)한 뒤 `lifecycle-int` 를 `lifecycle` 로 합쳤다(2eca9072).

**남은 항목 (검토에서 모음).**
- `clone:restore` 알림은 풀리는 계기가 없다 — 한 번 서면 그 프로젝트의 한 줄이 계속 `개발자에게 알렸어요` 다.
- `salvage/` · `repo.corrupt-*` 는 지우지 않는다 — 옛 클론의 node_modules 까지 남아 디스크를 두 배로 쓴다. 되살리기가 성공하면 며칠 뒤 지우는 규칙이 필요하다.
- `disk:low` 가 프로젝트마다 하루 한 번 올라간다 — 기계 전체 알림이므로 한 번이면 된다.
- 저장 · 넘기기 · 최신화 문제 해결 대화(dispatch 의 gateThreadFor)는 아직 Claude 만 띄운다 — `auto-thread.ts` 를 그대로 쓸 수 있다.
- `emitCycleEvent` 의 대체 경로가 `manager.list(cwd, 1)` 을 불러 사이드바 대화 목록이 다음 무효화까지 잘려 보일 수 있다(원래 있던 결함).
- whoAmI 캐시는 같은 토큰의 만료를 스스로 무효화하지 않는다(영향은 내 코멘트 거르기뿐).
- 베이스가 강제 푸시로 다시 쓰였으면 재클론의 되살리기가 실패해 `clone:restore` 로 간다.
- 반려 반영 턴은 대화를 열 수 없는 세계에서 10분 안팎에 예산을 다 쓴다.
- 준비 실패의 `clone` 안내(repo-guidance)가 AI 에게 git 으로 직접 clone 하라고 시킨다 — git 가드(L5)가 AI 의 참조 변경을 막으므로 할 수 없는 일이다.
- ScreenPanel 의 동기화 오류 · 세션 오류 배너는 대체 표 밖이라 남았다 — 영어 원문이 샐 수 있는 가장 가까운 자리다.
- 같은 이름의 두 프로젝트는 저장소 칩이 개발 실행 전용이 되어 실사용에서 구별되지 않는다 — 초대장이 이름을 겹치지 않게 해야 한다.
