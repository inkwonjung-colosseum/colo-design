...
| 10 | 보류 | 화면 정리 · 어휘 통일 · 제출 버튼 연결 · README. 사용자가 다른 세션으로 같은 화면 파일을 고치는 중이라, 그 작업이 정리된 뒤에 한다 |

통합은 `lifecycle-int` 브랜치에서 했다 — `lifecycle` 의 작업 트리에 사용자의 커밋되지 않은 화면 변경(`styles.css` 등)이 있어 그 트리에서는 병합이 거절됐다. 사용자의 변경이 커밋되면 `lifecycle-int` 를 `lifecycle` 로 합친다.
