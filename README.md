...
3. 앱이 쓰는 것을 함께 싣고 온다 — Node · pnpm 런타임에 git 까지(mac 은 이동식
   git 을, Windows 는 MinGit 을 싣는다). 관리자 권한은 한 번도 쓰지 않는다.
   화면을 만드는 AI 에이전트만 앱 안에서 준비한다: **Claude Code 는 준비 화면의
   설치 버튼 하나로 사용자 폴더에 설치되며 앱이 끝까지 지켜본다**(로그인은
   브라우저에서). **Codex 는 선택**이다 — 건너뛰어도 모든 일이 된다.
...
사이클에 보관된 작업이 있는지, GitHub 이 보고하는 제출한 요청 상태)라 사람이 임의로
바꿀 수 없다.
...
| `개발자가 보고 있어요` | 제출한 요청이 개발자에게 있다 — 검토 중 · 코멘트 도착 · 반려가 모두 이 색깔과 문장으로 읽힌다 |
...
길(버튼이나 안내)을 보여 준다. **준비 화면은 스스로 채워진다** — 설치 중에는 진행
줄이 한 줄씩 흐르고, 로그인은 브라우저에서 마치면 저절로 넘어간다(코드 붙여넣기 칸은
`브라우저가 열리지 않았나요?` 안에 있다). 회사 정책이 설치를 막으면 IT 담당자에게
그대로 보낼 문장이 나온다. **Codex 는 건너뛸 수 있는 선택 줄**이다. 모두 통과하면
`시작하기` 를 누르지 않아도 초대 파일 화면으로 저절로 넘어간다. 기존 사용자의 로그인이
만료되면 작업 화면은 그대로이고, 대화 맨 위의 문제 문장(`다시 연결이 필요해요`)에서
다시 로그인한다.

터미널은 열지 않는다. **에이전트 로그인은 앱 안에서 끝난다** — 로그인 버튼을 누르면
도구가 AI 의 로그인 명령을 대신 몰아 로그인 주소를 띄운다. git 은 두 OS 모두 앱이
함께 싣고 온다 — mac 의 시스템 git 설치 대화상자가 뜰 일도 없다.
...
받는다.
초대 파일로 온 프로젝트는 명령 실행이 늘 미리 허용돼 있으므로(형식 모듈의
`approveCommands: true`) 승인을 묻는 단계가 없다 — 처음 연결하는 저장소의 준비도
실패가 아니라 첫 실행으로 읽힌다(`이 서비스를 이 컴퓨터에서 처음 켭니다 — 준비에
몇 분 걸립니다`). 왼쪽 사이드바의 프로젝트를 고르면 그 서비스의
대화 · 미리보기 · 변경 사항이 오른쪽에 뜬다.
프로젝트 행에 마우스를 올리면 카드가 뜬다 — 프로젝트 이름과 지금 상태가 한 줄로 선다.
프로젝트를 지우는 길은 사용자에게 없다 — 지우면 제출하지 않은 작업까지 사라질 수
있어서다(지우기는 개발 실행의 개발자 도구에만 있다).
...
미리보기는 그 서비스의 앱을 그대로 띄운 것이다. AI 가 한 차례 답을 마치면 미리보기가
그 답이 고친 화면으로 스스로 옮겨 간다 — 이미 그 화면을 보고 있으면 그대로 둔다.
주소창을 누르면 이 대화에서 만든 화면이 제목으로 줄을 서고(글자를 치면 거른다),
평소에는 지금 화면의 제목이 경로 앞에 선다. 경로를 쳐도 그 화면으로 이동하고,
화면의 모든 컨트롤(검색 · 버튼 · 폼)은 실제 목 데이터와 동작한다.
뒤로 · 앞으로 · 새로 고침, 주소창, 모바일 · 태블릿 화면, 배율 조절이 다 있다. 짚을
곳이 없는데 화면이 이상하면 `보기 ▾` 의 `AI에게 이 화면 보여 주기` 로 화면 전체를
보낸다. 화면에
...
메모를 남길 수 있고, 여러 개를 찍은 뒤 문장을 붙여 한 번에 보낸다. 찍기가 켜진
동안은 막대 아래 `찍기 켜짐` 줄이 서고, 그 줄의 `끄기` 로 끈다 — 보내면 스스로
꺼진다. 핀과 문장은 한
턴이 되고, 보낸 핀은 그 자리에서 화면을 떠난다 — 해결 표식을 붙이고 다니는 일은
없다. 보낸 코멘트는 대화록에 카드로 남는다; 도구 어디에도 그것을 되읽는 목록은
없고, 다시 바라는 것이 있으면 그대로 대화에서 말하면 된다. 기록은 제출할 때 한 번
...
개발자가 본다. 누른 버튼이 스스로 답한다 — 도는 동안 `보내는 중…`, 끝나면 몇 초
`제출됐어요`. 잠긴 버튼을 누르면 이유가 버튼 아래 한 줄로 선다. 제출한 뒤에도
미리보기는 지금 화면 그대로다 — 보낸 시점의 화면은 막대의 `보낸 화면` 으로 본다.
이미 열린 요청이 있으면 제출은 같은 요청에 쌓인다. 진행은 도구가 지켜본다 —
창이 앞으로 오면 그 자리에서 확인하고, 대화가 없을 때도 늘 살핀다. 제출은 대화로도
된다 — "개발자에게 보내 줘"라고 분명히 말하면 AI 가 도구에게 제출을 부탁한다.
개발자가 코멘트를 남기면(리뷰에 달든 요청 밑에 그냥 달든) 도구가
스스로 반영한다 — AI 가 고친 뒤 같은 요청에 다시 실어 보내고, 완료 알림이 온다
(코멘트는 대화의 카드에서 곧장 `답하기`로 답할 수 있다 — E3). 제출이 무사히 끝나면 같은 자리에 영수증이 선다(E5): 보낸 내용
링크와 받을 개발자 수. 제출까지의 보관은 도구가 알아서 한다 — AI 가 한 차례 답할
때마다 그 시점이 `작업 기록`에 쌓이고, 제출은 그것을 묶어 보낸다.

### 7. 반영된 뒤 — 받아오기는 도구가 먼저 한다

요청이 합쳐지면(병합) 상태 칩이 `반영됐어요`가 된다. 개발자가 반영한 내용은 도구가
스스로 받아 온다 — 도구가 늘 지켜보다가(활성 프로젝트는 2 분마다, 그 외는 10 분마다)
반영을 보면 그 자리에서 받아 온다. 아직 보관되지 않은
...
- **잠긴 버튼** — 제출 버튼은 항상 그려지고, 못 쓰는 상태면 마우스를
  올릴 때 이유가 뜬다(예: `AI가 고치는 중 — 끝나면 제출할 수 있습니다`).
- **화면의 문제 문장은 셋이다** — `AI가 고치는 중이에요` · `개발자에게 알렸어요` ·
  `다시 연결이 필요해요`. 셋째만 사람의 손이 필요하다: 초대 파일을 새로 놓거나
  브라우저에서 다시 로그인하는 것, 각 한 번뿐이다. 그 외의 문제는 도구가 AI 나
  개발자에게 넘기고 그 사실을 이 한 줄로 말한다.
- **연결 레포의 오류는 AI 가 맡는다** — 설치가 넘어지거나, 미리보기 서버가 죽거나,
  화면에 오류가 나도 사람에게 오류 문장이나 명령어를 보여 주지 않는다. 도구가 그
  문제를 AI 에게 넘기고, 그 자리에는 `AI가 막힌 곳을 고치고 있어요` 같은 한 줄만
  선다. AI 가 두 번 고쳐 보고도 못 넘은 준비 실패는 개발자 알림(아래)으로 한 번 간다.
...
- **스스로 다시 시도** — 보낸 말이 일시적인 문제(전송 거절 · 일시 오류 · 잠시 채워진
  사용량)로 답을 못 받으면 도구가 같은 말로 알아서 다시 시도한다(4초 · 16초 · 1분 ·
  5분 · 15분의 간격으로 다섯 번). 대화에 그 사실이 한 줄 남고, 사다리를 다 쓰면
  실패 카드가 `고쳐서 다시 보내기`를 기다린다. 사용량이 다시 채워질 시각을 아는
  한도라면 그 시각에 맞춰 이어간다. 중지하면 재시도도 함께 멈춘다.
- **개발자 알림** — 연결 코드 만료 · 제출 실패 · AI 가 두 번 고쳐 보고도 못 넘은
  준비 실패처럼 AI 도 고칠 수 없는 문제가 생기면 도구가 개발자에게 알린다. GitHub 이
  먼저다 — 열린 요청이 있으면 그 요청에 코멘트로, 없으면 저장소에 이슈로. 슬랙은
  보조이며 초대 파일이 함께 싣는다(개발자용 설정에서 직접 붙일 수도 있다). 같은
  문제는 한 곳에만 쌓이고, 풀리면 도구가 그 글에 흔적을 남긴다.
...
계속하고, 대화에는 그 사실이 한 줄 남는다. 10분에 세 번을 넘게 되살려야 하는 세계
(망가진 프로그램)에서는 개발자에게 알리고, 화면에는 문제 문장 한 줄만 선다.
...
아니요. 데스크톱 앱에 Node · pnpm 런타임과 git 까지 들어 있고, 에이전트 설치·로그인도
앱 안에서 끝난다(Claude Code 는 준비 화면의 설치 버튼으로 사용자 폴더에 설치되고,
로그인은 브라우저에서 마친다). 터미널을 열 일이 없다 — 회사 정책이 설치를 막는 경우에만
IT 담당자에게 보낼 문장을 복사해 준다. Codex 는 선택 설치다.
...
git · 런타임을 지나 셋 다 통과하면 저절로 지나가고(경고가 남은 경우에만
`시작하기` 를 누른다), 프로젝트가 없는 시작 화면이 초대 파일을 기다린다. 개발
...
| `COLO_DESIGN_CODEX_RELEASE_API` | unset | Codex 릴리스 API 주소 — 검증용(로컬 서버가 JSON 을 내어 준다) |
...
턴마다 도구가 스스로 한다. 제출 버튼은 언제나 그려지고, 잠겼을 때 마우스를
...
둘은 GitHub · 미리보기로 가는 링크다. 브랜치와 클론 경로는 개발자의 어휘라 카드에서
치웠다(E10).
...
후보를 얻을 수 있다 — `파일 후보:` 줄의 `(관찰)` 표식이 그 출처다. 트레이의 행은 찍는
...
고른 것), git, Node·pnpm, 그리고 시작 화면의 초대 파일. 각 실패는 한국어로 이유를
...
첫 실행은 초대 파일을 요구한다 — 시작 화면이 그 한 장으로 서 있다(개발 실행도
같은 길이다). 초대장이 실어 온 연결 코드의
...
초대장에 이름이 없을 때만 시작 화면의 확인 카드가 선택 칸으로 묻는다. 봇 계정으로 열리는
풀 리퀘스트의 본문(`> 작성: <이름>`)과
...
다). 마법사가 Windows 에서 bash 를 확인하고 없으면 앱 재설치를 안내한다. 이 판단 전부는
...
| `packages/web` | 사용자(비개발자) UI — 홈(받은 편지함), 프로젝트 사이드바, 초대 파일 가져오기, 채팅, 레포 미리보기(주소창 · 핀 · 배율 · 단축키 시트), 상단 바의 제출 · 상태 확인, 작업 기록 드로어, 온보딩 마법사, 설정. |
...
3. 준비 화면 — Claude Code 설치 버튼이 진행 줄을 흐르게 하고, 로그인은 브라우저에서
   마치면 저절로 다음으로 넘어간다. `시작하기` 를 누르지 않아도 초대 파일 화면이 뜬다.
...
```bash
pnpm test                   # 빌드 뒤 단위 시험 — 데몬(명령 멱등 · 실패 분류 · 턴 통계 · 로그 새니타이저) · 웹의 순수 판정(턴이 말한 화면)
