...
요청에 다시 실어 보내고, 완료 알림이 온다(코멘트는 대화의 카드에서 곧장 `답하기`로
답할 수 있다 — E3). 제출이 무사히 끝나면 같은 자리에 영수증이 선다(E5): 넘긴 내용
...
**초대 파일** — 개발자가 `node scripts/make-invite.mjs --repo … --name … --token …
[--author "이름"] [--reviewer 로그인]…` 으로 사용자마다 한 번 만들어 주는
`*.colo-invite`(JSON, v2). 시작 마법사의 열기 버튼이나 창 전체 드래그&드롭으로 받아
`github.token.set` → `project.create` 를 차례로 보내고, 성공하면 원본 파일 삭제를
제안한다. v2 의 `--author` 는 넘긴 요청의 `> 작성:` 줄에 적힐 이름을 미리 정하고,
`--reviewer`(반복 가능)는 이 프로젝트의 넘기기가 GitHub 에 리뷰를 부탁할 개발자들이다
(E4) — 사용자가 리뷰어를 고르는 일은 없다. URL 스킴을 쓰지 않는 이유는 하나다 — `colo-design://…?token=…`
