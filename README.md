...
입력은 뭐든 된다. 기획서 파일을 첨부할 수도 있고, 그림을 붙여넣을 수도 있고, 그냥 말로
시작할 수도 있다. 기획서는 가장 빠른 길 하나일 뿐, 이 도구가 요구하는 것은 아니다.
...
```mermaid
flowchart LR
    ask["무엇을 만들지<br/>(말 · 그림 · 기획서)"] --> chat["화면 대화<br/>(Claude 세션)"]
...
기획서 파일을 첨부하거나 그림을 붙여넣으면 더 빠르지만, 없어도 된다. 결과가 아니라
