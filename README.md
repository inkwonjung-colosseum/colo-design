...
만료되면 작업 화면은 그대로이고 상단 배너에서 다시 로그인한다.
...
이미 열린 요청이 있으면 제출은 같은 요청에 쌓이고, 진행은 `상태 확인`
...
- **개발자 부르기** — 도구도 AI 도 고칠 수 없는 자리(데몬 환경의 API 키)에서는 카드에
  `개발자 부르기` 가 선다. 누르면 개발자의 슬랙으로 상황이 그대로 간다(설정 → 알림에
  연결돼 있을 때. 아니면 이유와 함께 잠긴다).
...
- **스스로 다시 시도** — 보낸 말이 일시적인 문제(전송 거절 · 일시 오류 · 잠시 채워진
  ...
  재시도도 함께 멈춘다.
- **개발자 알림 (Slack)** — 연결 코드 만료 · 넘기기 실패 · AI 가 두 번 고쳐 보고도 못
  넘은 준비 실패처럼 AI 도 고칠 수 없는 문제가 생기면 개발자의 슬랙으로 알린다. 설정 → 알림의 `개발자 알림` 에 웹훅 주소 또는
