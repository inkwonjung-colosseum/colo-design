...
- **스테퍼.** 각 화면이 어디까지 왔는지가 단계로 선다: 화면 만들기 → 검토·수정
  → 저장 → 개발자에게 넘기기 → 반영됨. 무엇을 하면 되는지는 주 버튼 하나가
  말하고, 판정 근거가 한 줄로 붙는다. 판정은 전부 기계적 신호다 — 레포가 선언한
  화면, 클론의 저장 안 한 변경 건수, GitHub 이 보고하는 풀 리퀘스트. 일이
  좋은지는 묻지 않는다. 저장 안 한 변경 건수는 클론 준비·화면 턴 종료·저장
  완료 세 트리거로 갱신된다(폴링 없음).
...
- 데몬과 UI 사이의 선은 `packages/protocol` (v8) 이다: zod 로 검증되는 클라이언트
...
| 패키지 | 하는 일 |
| --- | --- |
| `packages/protocol` | v9 선로 계약(zod 로 검증되는 클라이언트 메시지), 공유 수동 업데이트 확인 로직, 기계가 쓴 턴을 카드로 그리게 하는 턴 마커. |
| `packages/daemon` | 프로젝트 레지스트리, 세션, 레포 워크스페이스(clone · 저장/넘기기 게이트), 온보딩 검사, 자격 증명 저장, 데스크톱을 위한 정적 웹 서빙. |
| `packages/web` | 기획자 UI: 프로젝트 사이드바, 레포 피커(프로젝트 추가), 세션 탭 스트립, 채팅, 레포 미리보기, 스테퍼, diff 검토, 온보딩 마법사, 설정. |
