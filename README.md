...
| 상태 칩 | 뜻 |
| --- | --- |
| `변경 없음` | 이번 사이클에 새로 만든 것이 없다 |
| `만드는 중` | AI 가 도는 중이다 — 어느 상태에 있든 이 말이 앞선다 |
| `제출 전` | 만든 것이 이 컴퓨터에 보관돼 있다. 아직 개발자에게 가지 않았다 |
| `개발자 검토 중` | 넘긴 요청을 개발자가 보고 있다 |
| `변경 요청` | 개발자가 넘긴 요청에 코멘트를 남겼다 — 도구가 코멘트를 읽고 AI에게 반영을 맡긴 뒤, 고친 화면을 같은 요청에 다시 실어 보낸다 |
| `반영됨` | 작업이 제품에 합쳐졌다. 다음에 만드는 것은 새 사이클이다 |
| `개발자가 반려함` | 개발자가 넘긴 요청을 닫았다 — `상태 확인`으로 이유를 읽고, 고쳐 제출하면 새 요청이 열린다 |

## 사용 안내 — 일곱 걸음
### 1. 처음 켤 때 — 준비 확인

앱을 처음 열면 네 가지를 확인한다: 에이전트 로그인(설정에서 고른 AI), git, 런타임(앱에
포함 — 없다고 나오면 앱 설치가 깨진 것), **연결 코드**(GitHub 접근용 코드. 개발자가
발급해 준다). 하나라도 안 되면 한국어로 이유를 말하고 고치는 길(버튼이나 안내)을
보여 준다.
...
개발자가 **초대 파일**(`*.colo-invite`)을 보내 줬다면 시작 화면에서 그 파일을 열거나
창에 끌어다 놓는 것으로 연결 코드 저장과 프로젝트 만들기가 한 번에 끝난다. 파일에는
비밀이 들어 있으니 가져온 뒤에는 지운다(화면이 그렇게 안내한다).

연결 코드가 없어도 시작은 할 수 있다 — 공개 저장소로 작업하는 길은 열려 있고,
저장소 목록과 개발자에게 넘기기만 쓸 수 없게 된다. 코드가 이 레포에 닿지 못하면
도구가 그렇게 말한다(개발자에게 다시 요청하라고).

### 2. 프로젝트 고르기 — 작업할 서비스

프로젝트는 곧 작업할 서비스 하나다. `프로젝트 추가` 목록에서 토큰이 접근할 수 있는
저장소를 고르거나, 목록에 없으면 `주소로 추가`로 직접 넣는다. 처음 연결하는 저장소가 뭔가를
설치하고 미리보기를 띄우는 일은 사용자의 승인 한 번(`준비 시작`) 뒤에만 돈다 — 화면은
이것을 실패가 아니라 첫 실행으로 말한다(`이 서비스를 이 컴퓨터에서 처음 켭니다 —
준비에 몇 분 걸립니다`). 승인은 기억되어 다시 묻지 않는다. 왼쪽 사이드바의 프로젝트를 고르면 그 서비스의 대화 ·
미리보기 · 변경 사항이 오른쪽에 뜬다.
프로젝트 행에 마우스를 올리면 카드가 뜬다 — 저장소, 이번 작업이 쌓이는
보관함(브랜치), 돌고 있는 미리보기 주소, 이 기계의 작업 폴더가 줄로 서고, 누르면 그
자리로 가거나 폴더가 열린다.
...
참고 그림이나 문서 파일을 함께 붙이면 더 빠르지만, 말만으로도 된다. 첫 문장이 떠오르지
않으면 빈 대화가 권하는 예시 세 마디를 눌러 입력창에 담고 고쳐 써도 된다.

입력창의 설정 칩(`방식 · 모델 · 생각` 요약)에서 `확인 방식` 도 고른다 — 이름은 고르면
무슨 일이 일어나는지 그대로 말한다.

| 확인 방식 | 무엇이 일어나는가 |
| --- | --- |
| `바로 진행` (기본) | 아무것도 묻지 않는다 |
| `실행 전에 물어보기` | 명령을 돌리기 전에 카드로 물어본다 |
| `화면 수정은 바로` | 안전한 수정은 묻지 않고 진행한다 |
| `계획 먼저 보기` | 만들기 전에 무엇을 만들지 보여 준다 |

결과가 아니라 뼈대부터 보고 싶으면 `계획 먼저 보기` 로 정해 보낸다 — AI 의 첫 답은
`만들 것` 카드로 오고, 승인하면 착수한다. `바꿔 달라…`로 거절하면 이유를 갖고 다음
계획을 다시 받는다. 확인 카드가 뜰 때, 레포가 정한 명령은 이름으로 읽힌다
(`레포 검사` · `설치 실행` — 날 명령줄은 `자세히 보기` 안에 있다).
...
버튼으로 본다. 개발자가 코멘트를 남기면 도구가 스스로 반영한다 — AI 가 고친 뒤 같은
요청에 다시 실어 보내고, 완료 알림이 온다(`상태 확인`에서 코멘트를 읽고 `답하기`로
직접 답할 수도 있다). 제출까지의 보관은 도구가 알아서 한다 — AI 가 한 차례 답할
...
시작할 때와 화면 막대의 `최신 변경 받아오기` 버튼으로 받아 온다. 아직 보관되지 않은
작업이 있으면 잠시 치워 뒀다가 그 위에 다시 얹는다 — 겹치지 않는 편집은 자동으로
합쳐지고, 정말 겹치는 부분만 AI 의 첫 과제가 된다.

### 마음이 바뀌면 — 되돌리기

- 한 요청이 끝난 줄(정산 줄)의 `여기서 새 대화` — **대화만** 이 답까지로 이어받는다.
  화면은 지금 모습 그대로다(워크트리는 하나이므로 분기는 기억만 갈라 낸다). 그 사실이
  줄 위에 항상 적혀 있고, 화면까지 되돌리고 싶은 사람을 위해 같은 줄에
  `작업 기록에서 되돌리기` 링크가 선다. 지금 대화는 목록에 그대로 남으므로 한
  지점에서 두 갈래로 이어해 볼 수 있다. 같은 줄 오른쪽에는 소요 시간과 `전체 복사` 가 선다.
...
- **에이전트 고르기** — 입력창의 설정 칩 팝오버 맨 윗줄 `프로바이더`(또는 설정의
  ...
  단계는 고름이 다음 새 대화부터 먹는다는 노트를 먼저 읽혀 둔다. 설치되지 않은
  에이전트는 행에 이유가 적힌다.
...
- **첫 안내** — 처음 쓰는 기계에서는 안내가 한 번에 하나씩, 순서대로 뜬다:
  빈 대화의 예시 → 미리보기의 `수정할 곳 찍기` → 상단 바의 `제출`. 한 번 지나가면
  다시 오지 않는다.
...
- **자동 보관(커밋)** — 답을 낸 턴이 끝날 때마다 데몬이 스스로 커밋한다. 첫 커밋이
  ...
  돌리면 구독이 두 배로 탄다). 푸시는 백그라운드이고 실패를 삼킨다: 오프라인에서
  턴의 끝이 멈추지 않는 대신, 제출 전까지 원격 백업이 보장되지 않는 순간이 생긴다.
...
버튼으로 받아 온다. 아직 커밋되지 않은 변경이 있으면 임시 보관한 뒤 클론을 최신으로 옮기고 그 위에
...
프로젝트 행에 마우스를 올리면 카드가 뜬다 — 저장소, 이번 주기가 커밋을 쌓는
브랜치(첫 커밋 전에는 기본 브랜치), 살아 있는 미리보기 주소, 클론 폴더. 앞의 셋은
GitHub · 미리보기로 가는 링크고, 클론 폴더 줄은 `project.openFolder` 로 데몬이
플랫폼의 여는 도구를 떼어 놓아 실행한다 — 응답은 접수 사실만 말한다.
...
둘뿐이다 — 문서가 완전히 로드됐는지와 콘솔의 오류 · 실패한 요청. 경고는 세지 않는다(레포 개발 빌드의 기본 소음이라 게이트가 그것으로
...
주소를 받아 검증 창에서 그 화면을 열어 보고 문서 완료 로드와 콘솔 오류만 돌려준다
...
멈춰도 도구는 산다. 주소창(전체 주소를 보여 주고, 미리보기 서버 안의 주소만 받는다),
뒤로 · 앞으로, 새로 고침(보고 있는 자리 그대로), 오류 배너, 모바일 · 태블릿 실제
...
연결 코드가 없는 것은 차단이 아니라 주의다 — 공개 레포나 주소를 직접 넣는 길은
코드 없이도 열려 있고, 없으면 레포 목록과 개발자에게 넘기기만 막힌다. 게이트는
`whoAmI` 뿐 아니라 **쓰기 가능한 레포 수**도 본다: 0 개면 "개발자에게 받은 코드가 이
레포에 닿지 않습니다" 로 말한다. 온보딩 마지막 줄에는 **이 작업에 적을 이름** 하나가
있다 — 봇 계정으로 열리는 풀 리퀘스트의 본문(`> 작성: <이름>`)과 커밋 identity 가 그
이름을 쓴다. 어느 레포로 일할지는 게이트가 아니다 — 프로젝트는 작업 화면에서 코드가
접근할 수 있는 레포 목록에서 골라 추가하고, 내려받기 · 설치 진행은 미리보기 칸이
그린다. 코드는 기계에 하나이고 OS 자격 증명 저장소로 간다 — 데스크톱 앱에서는
Electron `safeStorage` 경유 키체인, 그 외에는 `security` CLI — 그리고 선상에는 존재
여부만 건넌다.

**초대 파일** — 개발자가 `node scripts/make-invite.mjs --repo … --name … --token …` 으로
사용자마다 한 번 만들어 주는 `*.colo-invite`(JSON). 시작 마법사의 열기 버튼이나 창 전체
드래그&드롭으로 받아 `github.token.set` → `project.create` 를 차례로 보내고, 성공하면
원본 파일 삭제를 제안한다. URL 스킴을 쓰지 않는 이유는 하나다 — `colo-design://…?token=…`
은 토큰이 프로세스 argv 와 OS 로그에 남지만, 파일 경로만 argv 에 남는 초대 파일은
비밀이 파일 안에 있다. OS 파일 연결(단계 B)은 아직 없다.
...
- 처음 연결하는 레포의 `install` · `preview` 명령은 사용자의 승인 한 번 뒤에만
  돈다 — 프로젝트 추가 화면의 확인란이거나, 승인 없이 멈춘 준비 화면의 `준비 시작`
...
| `packages/web` | 사용자(비개발자) UI — 홈(받은 편지함), 프로젝트 사이드바, 레포 피커(프로젝트 추가), 채팅, 레포 미리보기(주소창 · 핀 · 배율 · 단축키 시트), 상단 바의 제출 · 상태 확인, 작업 기록 드로어, 온보딩 마법사, 설정. |
