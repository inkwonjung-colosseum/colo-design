...
입력은 말과 그림이다. 만들고 싶은 화면을 그냥 말해도 되고, 참고할 그림을 붙여넣어도
되고, 적어 둔 내용이 있으면 대화창에 그대로 붙여넣어도 된다.
...
```mermaid
flowchart LR
    ask["무엇을 만들지<br/>(말 · 그림)"] --> chat["화면 대화<br/>(AI 세션)"]
...
그림을 붙여넣으면 더 빠르지만, 말만으로도 된다. 결과가 아니라
