...
**비개발자를 위한 화면 만들기 도구다.** git이나 Claude Code, 터미널을 한 번도 써 본 적
...
3. 앱에는 실행에 필요한 런타임(Node · pnpm)이 들어 있다. 따로 준비할 것은 둘 —
   화면을 만드는 AI 에이전트 하나(Claude Code · Codex · OpenCode · omp 중
   설치하고 로그인한 것)와 git.
...
| `저장 안 함` | 저장하지 않은 작업이 있다. AI 가 고치는 중이면 `고치는 중`으로 읽는다. 넘긴 요청이 열려 있으면 저장분은 그 요청에 함께 담긴다 |
...
새 대화는 사이드바의 ＋, 행 메뉴, ⌘T 어디로든 연다. 원하는 화면을 말로 보낸다 —
참고 그림이나 문서 파일을 함께 붙이면 더 빠르지만, 말만으로도 된다. 결과가 아니라
뼈대부터 보고 싶으면 입력창의 `확인 방식` 칩을 `Plan` 으로 골라 보낸다 —
AI 의 첫 답은 `만들 것` 카드로 오고, 승인하면 착수한다. `바꿔 달라…`로 거절하면
...
정말 겹치는 부분만 AI 의 첫 과제가 된다.

### 마음이 바뀌면 — 되돌리기

- 한 요청이 끝난 줄(정산 줄)의 `이 요청 이전으로 되돌리기` — 화면 파일만 그 요청이 시작하기
  전 모습으로 돌린다. 되돌리기도 기록에 쌓이므로 지금까지의 역사는 지워지지 않는다.
- 같은 줄의 `다시 요청` — 답을 통째로 버리고 대화와 파일을 그 전으로 돌려 같은 말로
  다시 받는다. 행동은 줄 왼쪽에, 소요 시간과 `전체 복사` 는 오른쪽에 있다.
...
- **에이전트 고르기** — 새 대화의 입력창 위 `에이전트` 칩(또는 설정의 에이전트
  목록)에서 이 대화가 어느 AI 로 돌지 고른다. 고른 것이 곧 설정에 남고, 이미 열린
  대화는 태어난 에이전트를 그대로 유지한다. 설치되지 않은 에이전트는 행에 이유가
  적힌다. 모델 목록은 대화를 시작하기 전에도 채워져 있다 — 데몬이 각 CLI 에서
  미리 읽어 온다(omp · OpenCode).
- **알림** — AI 가 작업을 마치거나 사용자 확인을 기다리거나 저장이 끝나지 못해 대화가
...
아니요. 데스크톱 앱에 필요한 런타임이 들어 있다. 에이전트 CLI 하나와 git만
설치하고 로그인하면 된다 — 에이전트는 Claude Code · Codex · OpenCode · omp 중
고른다.
...
**요구 사항** — 로그인된 에이전트 CLI 하나 이상(Claude Code `claude /login`,
Codex, OpenCode, omp), git, 그리고 Claude 를 쓸 때는 데몬 환경에
...
| `~/.colo-design/config/undo.jsonl` | 되돌리기 측정 — 다시 요청 · 턴 되돌리기 · 저장 되돌리기의 종류 · 프로젝트 · 턴 번호만. 사용자의 말도 화면도 남기지 않는다 |
...
답이 어긋난 턴은 답변 카드에서 두 갈래로 되돌린다. `이 요청 이전으로 되돌리기` 는
화면 파일만 그 요청이 시작하기 전 모습으로 돌린다 — 되돌리기도 새 커밋이므로 역사는
...
세 단어가 git 명사 전부를 대신한다. 사용자는 브랜치, 커밋, 푸시, PR, 머지를 읽지 않는다.
...
프로젝트 행에 마우스를 올리면 카드가 뜬다 — 저장소, 이번 주기가 저장을 미는
브랜치(첫 저장 전에는 기본 브랜치), 살아 있는 미리보기 주소, 클론 폴더. 앞의 셋은
...
핀은 어느 페이지에나 찍힌다 — `[data-screen]` 래퍼가 없는 페이지(아직 이
도구로 만지지 않은 레포의 원래 화면)에서도 ⌥+클릭이 핀을 남기고, 그 페이지의 경로가
화면 id 가 된다. 코멘트 → 수정의 고리는 처음부터 열려 있다.
...
둘뿐이다 — 화면이 자리를 잡았는지(요청한 상태의 표식이 나타났는지)와 콘솔의 오류 ·
실패한 요청. 경고는 세지 않는다(레포 개발 빌드의 기본 소음이라 게이트가 그것으로
...
새 화면은 결과가 아니라 의도부터 검토한다. 입력창의 `확인 방식` 칩을 `Plan` 으로
골라 보내면 대화가 계획 자세로 서고, AI 의 첫 답변은 `만들 것` 카드로 온다 — 무엇을
...
연결 레포는 자기가 그릴 수 있는 것을 선언한다 — 라우트, 제목, `states`. 미리보기
브리지가 그 목록을 `colo-design.screens` 봉투로 도구에 올린다(데스크톱에서는 도구의
preload 가 내놓은 `window.coloDesign.post` 문으로, 브라우저 개발 경로에서는
postMessage 로). 미리보기 툴바의 선택기가 이 목록 그 자체다 — 기능별로 묶여, 화면을
고르면 미리보기가 그 화면으로 이동하고, 상태 칩은 `empty` 나 `error` 를 실제 목
데이터로 그린다. 화면의 상태(`변경 있음` / 넘김 / 반영됨)는 diff 와 풀 리퀘스트에서
기계적으로 파생된다. 화면이 말한 대로 만들어졌는지가 이 도구가 하지 않는 유일한
판단이다 — 그 답은 미리보기를 보는 사람에게 있고, 대신 판정하는 것은 이 도구의 일이
아니기 때문.
...
Node 는 링크로만 제시한다 — 도구가 사용자의 기계에 설치를 실행하지는 않는다(데스크톱
...
데몬과 UI 사이의 선은 `packages/protocol` (v15) 이다. zod 로 검증되는 클라이언트
...
같은 원칙이 다른 에이전트에도 그대로다 — Codex · OpenCode · omp 도 각 사용자가
