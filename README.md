...
화면은 주소다. 레포의 페이지가 루트에 `data-screen="member/MemberList"` 표식을
남기고, 화면의 상태는 같은 자리의 `data-state` 표식 — `?state=empty` 주소 문법으로
그 상태를 실제 목 데이터로 그린다. 주소창이 이 계약의 전부다: 경로를 치면 그 화면으로
이동하고, `?state=` 를 붙이면 그 상태로 그린다. 화면의 상태(`변경 있음` / 넘김 /
반영됨)는 diff 와 풀 리퀘스트에서 기계적으로 파생된다. 화면이 말한 대로 만들어졌는지가
이 도구가 하지 않는 유일한 판단이다 — 그 답은 미리보기를 보는 사람에게 있고, 대신
