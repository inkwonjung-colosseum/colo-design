...
/**
 * The "Claude에게 해결 요청" payload. `step` names the failed step in the
 * transcript card's words (`<step>에서 멈췄습니다`), `thread` names the
 * conversation the tool opens, `brief` is what Claude reads — the failure
...
export interface Guidance {
  ...
  command?: string;
  /**
   * 카드의 첫 동작 — 이 실패를 Claude 의 과제로 넘긴다. 없는 종류는
   * `commands` 하나다: 승인은 사람의 동의라 Claude 의 몫이 될 수 없고,
   * 실행 허용 버튼이 그 자리를 지킨다. `preview` 도 없다 — 죽은 서버의
   * 카드는 미리보기 자리가 자기 Claude 버튼을 이미 그린다.
   */
  claude?: ClaudeAsk;
}

const ask = (step: string, thread: string, lead: string, detail: string | null): ClaudeAsk => ({
  step,
  thread,
  brief: detail ? `${lead}\n\n${detail}` : lead,
});
...
export function guidanceFor(kind: ErrorKind, detail: string | null): Guidance {
  ...
  if (kind === "port-busy") {
    ...
    // 직접 종료 또는 포트 변경이다 — 그 일을 Claude 의 과제로 넘긴다.
    return {
  ...
  if (kind === "held-elsewhere") {
    ...
    // 인스턴스를 끄는 일이라, Claude 도 그 프로세스를 죽이는 대신 이
  ...
  if (kind === "install") {
    ...
    return {
      ...
      body:
        detail ??
        "연결 레포의 의존성 설치가 실패했습니다 — Claude에게 해결을 요청하면 대화에서 원인을 찾아 고칩니다.",
      claude: ask(
        "의존성 설치",
        "설치 실패 해결",
  ...
  if (kind === "bootstrap") {
    // 준비 턴(레포를 살펴 colo-design.json 을 계약에 맞게 채우는 Claude 의
    // 첫 작업)이 못 끝낸 상태 — 같은 일을 대화에서 이어 받는다.
    return {
      title: "연결 준비를 마치지 못했습니다",
      body:
        detail ??
        "레포 연결 준비가 끝나지 않았습니다 — Claude에게 해결을 요청하면 대화에서 이어 갑니다.",
      claude: ask(
        "연결 준비",
        "연결 준비 해결",
        "레포 연결 준비(레포를 살펴 colo-design.json을 계약에 맞게 채우는 작업)가 끝나지 않았습니다. 레포를 살펴 colo-design.json을 계약에 맞는 설정으로 고쳐 주세요.",
        detail,
      ),
    };
  }
  if (kind === "preview") {
    ...
    // 그 카드가 자기 "Claude에게 고쳐 달라고 하기" 를 이미 그리므로 여기의
    // claude 는 비워 둔다 (두 문이 하나의 실패를 두고 다투지 않게).
    return {
      title: "준비하지 못했습니다",
