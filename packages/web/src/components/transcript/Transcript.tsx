...
export function Transcript({
  ...
  /**
   * 사람 메시지의 `답하기` — 컴포저를 답장 모드로 연다(states.md §2.2).
   * 보내는 길(`api.replyToReview`)은 ChatColumn 이 쥔다.
   */
  onReplyReview?: (review: DeveloperReview) => void;
  /**
   * 게이트 사이의 복도(states.md §2.2 저장 카드의 나가는 길): 마지막 저장
  ...
  /** 접어 둔 복도 — `아직이요` 를 누른 저장 카드의 id. 창의 기억이다(states.md §2.2). */
      ...
      {rows.map((row) => {
        ...
        switch (block.type) {
                        ...
                        >
                          <button
                            ...
                            onClick={() =>
                              askRewind(
                                turnNo,
                                lastUserText(blocks) ?? turnAnswers.get(block.id) ?? "",
                              )
                            }
                          >
                            다시 요청
              ...
              <div key={block.id}>
                ...
                {corridor && (
                  <div className="corridor">
                    <button type="button" className="corridor__go" onClick={saveCorridor.onHandoff}>
                      개발자에게 넘기기로 이어가기
                    </button>
                    <button
                      type="button"
                      className="corridor__later"
                      onClick={() => setCorridorDismissed((prev) => new Set(prev).add(block.id))}
                    >
                      아직이요
                    </button>
                  </div>
          ...
          case "human": {
            ...
            // 행 — §5 엣지케이스 5: 핀 매핑이 없으니 라벨은 코드 위치뿐이다.
            ...
            return (
              <HumanMessage
                ...
                onFixAll={onFixReview && inline.length > 1 ? () => onFixReview(inline) : undefined}
              />
            );
          }
          default:
