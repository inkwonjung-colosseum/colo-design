 ...
 * the marker card it carries (PLAN D9). 핀 턴은 재설계 C2·C7 을 따른다:
...
import type { PinAttachment } from "./usePins";
...
/**
 * The pins turn (재설계 C2): N pins plus the composer's sentence become
 * ONE turn. The marker carries the card (one screen's title, or 화면 N곳
 * when the batch spans screens, C6); the body is the sentence, one guide
 * line, and one readable block per pin — what was pinned, the repo's own
 * source stamp (C8), the memo, the React owner chain and the CSS path with
 * the pin-time rect (C9), which is all Claude ever needed the json fence
 * for (C7).
 *
 * The intent (재설계 C10) words the batch: all questions ask to be explained
...
export function pinsToTurn(
  ...
  // 영역 핀의 이름표 (재설계 C9): a region has no words of its own — its
  ...
  const marker: TurnMarker = {
    ...
    items: pins.map((pin) => ({
      // The pin's own key (커미티 2차 판정 5): the card row joins back to the
      ...
      // The card titles the mix; absent reads as change (재설계 C10).
  ...
  const blocks = pins.map((pin, index) => {
    ...
    // 계산된 스타일의 일부 (재설계 C9) — the planner saw these values.
    const styleRows = Object.entries(pin.element.styles ?? {})
      .slice(0, 6)
      .map(([key, value]) => `${key} ${value}`);
    if (styleRows.length > 0) rows.push(`   스타일: ${styleRows.join(" · ")}`);
    // outerHTML (재설계 C9) — one line; the markup is context, not a file.
    if (pin.element.html) {
  ...
  const lines = [
    // 문장이 있으면 첫 줄 — 마커의 note 와 같은 말이다 (재설계 §3.8).
    ...(sentence ? [sentence, ""] : []),
    allQuestion
