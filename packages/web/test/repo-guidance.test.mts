...
test("errorKindOf: kind 없는 옛 데몬은 detail 의 다른 프로그램 문구로 포트 충돌을 읽는다", () => {
  const kind = errorKindOf(
    status({
      errorKind: null,
      detail: "포트 3000를 다른 프로그램이 이미 쓰고 있어 미리보기를 켤 수 없습니다 — 다시 시작을 누르면 그 프로그램을 종료하고 미리보기를 다시 켭니다.",
    }),
  );
  assert.equal(kind, "port-busy");
});
...
test("errorKindOf: 갈라진 거부는 포트 충돌이 아니다", () => {
  const kind = errorKindOf(
    status({
      errorKind: null,
      detail: "기본 브랜치에 원격과 갈라진 커밋이 있어 자동 최신화를 멈췄습니다 — 대화를 열면 Claude가 확인합니다.",
    }),
  );
...
test("errorKindOf: clone 실패는 그 종류로, install 실패는 알 수 없음으로 — 카드는 detail 을 그대로 보여 준다", () => {
  assert.equal(errorKindOf(status({ errorKind: "install", detail: "설치가 실패했습니다" })), "unknown");
  assert.equal(errorKindOf(status({ errorKind: "clone", detail: "내려받기가 실패했습니다" })), "clone");
});
...
test("guidanceFor: 충돌·승인 카드의 첫 동작 문구는 그대로다 (회귀)", () => {
  ...
  assert.equal(guidanceFor("auth", null).command, "pnpm config set //npm.pkg.github.com/:_authToken <PAT>");
