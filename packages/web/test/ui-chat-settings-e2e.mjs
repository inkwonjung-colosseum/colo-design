...
function instantStubClaude(dir) {
  ...
  writeFileSync(
    ...
    [
      ...
      // initialize 에 모델 행으로 답한다 — 칩의 요약(무엇으로 · 얼마나 생각
      // · 얼마나 자유)은 모델 행이 있어야 "·"로 선다. 답하지 않으면 세션의
      // models 는 늘 비어, 깨끗한 기계(러너)에서 칩이 영영 "바로 진행"만
      // 말했다(CI 2026-09-21). 로컬 통과는 예전 실행이 localStorage 카탈로그
      // 에 남긴 행에 기댄 환경 의존이었다.
      '    if (line.includes(\'"subtype":"initialize"\')) {',
