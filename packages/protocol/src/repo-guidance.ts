...
export function guidanceFor(kind: ErrorKind, detail: string | null): Guidance {
  ...
  if (kind === "conflict") {
    ...
    return {
      ...
      body:
        detail ??
        "보관하지 않은 변경과 개발자의 최신 변경이 겹쳤습니다. AI에게 정리를 요청하면 대화에서 충돌을 정리합니다.",
      agent: ask(
        "최신 변경 받아오기",
        "최신화 충돌 정리",
        "준비가 최신화 충돌로 멈춰 있습니다. 충돌을 정리해 보관 전 상태로 돌려 놓고, 미리보기가 다시 뜨도록 준비를 마쳐 주세요.",
        detail,
      ),
    };
  }
  ...
  if (kind === "clone") {
    // AI 는 git 쓰기를 하지 못한다(PLAN L5 — 세션의 git 가드가 참조 변경을
    // 막는다). 그러니 브리프는 "직접 clone" 을 시키지 않고, 원인을 좁히고
    // 고칠 수 있는 것(설정 파일 등)만 고치게 한다. 복구 턴이 끝나면 도구가
    // 다시 받는다(bring-up 재시도) — 그 사실을 AI 에게도 알려 헛된 git 시도를
    // 미리 막는다.
    return {
      title: "준비하지 못했습니다",
      ...
      agent: ask(
        "레포 내려받기",
        "내려받기 실패 해결",
        "연결 레포를 내려받지 못해 준비가 멈춰 있습니다. git 명령 없이 아래 출력에서 원인(네트워크 · 주소 · 권한)을 확인해 주세요. 도구가 다시 받을 수 있게 고칠 수 있는 것(예: 설정 파일)은 고치고, 사람의 권한이 필요한 일이면 무엇이 필요한지 한국어로 알려 주세요. 정리가 끝나면 도구가 스스로 다시 받습니다.",
        detail,
      ),
    };
  }
