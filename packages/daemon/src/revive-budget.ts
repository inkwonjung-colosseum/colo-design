...
import { MAX_AUTO_REVIVES } from "./turn-retry.js";

/** 되살리기를 세는 창 — 이보다 오래된 시도는 잊는다. */
const WINDOW_MS = 10 * 60_000;

export class ReviveBudget {
  private readonly attempts = new Map<string, number[]>();

  constructor(private readonly max: number = MAX_AUTO_REVIVES) {}

  /** 허용이면 셈을 올린다 — 거절하면 아무 것도 바뀌지 않는다. */
  allow(sessionId: string, now: number): boolean {
    const fresh = (this.attempts.get(sessionId) ?? []).filter((at) => now - at < WINDOW_MS);
