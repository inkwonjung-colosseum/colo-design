 ...
 * 실패하면 백업을 되돌려 앱 부재 상태로 끝나지 않게 한다(sha256 은 zip
 * 무결성만 증명한다 — 디스크 만석이나 권한은 여기서 실패한다). 모든 출력은
...
rm -rf "$BACKUP"
mv "$TARGET" "$BACKUP" || { echo "기존 앱을 백업으로 치우지 못했습니다"; exit 1; }
...
/usr/bin/open "$TARGET" || { echo "새 앱 실행에 실패했습니다"; exit 1; }
rm -rf "$BACKUP"
