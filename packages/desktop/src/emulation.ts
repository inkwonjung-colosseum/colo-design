 ...
 * (PLAN D69, `PlannerPreviewView.emulate`); Claude's hidden window narrows
 * with CDP `Emulation.setDeviceMetricsOverride` (PLAN D61, the preview
 * driver). Two APIs, but the same three widths — a screen Claude called
...
export const VIEWPORT_METRICS: Record<"mobile" | "tablet" | "desktop", ViewportMetrics> = {
  ...
  /** The default window — what Claude's driver opens with. */
