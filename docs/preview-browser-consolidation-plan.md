...
- Orca/Paseo식 단일 공유 브라우저(에이전트와 사용자가 같은 WebContents를 봄)는 **에이전트의 조작 난이도나 호출 수를 개선하지 않는다** — 사용자 가시성과 로그인 세션 공유를 위한 제품 결정이며, 게이트의 무오염 재검증과 상충한다. 3단계는 구현 계획이 아니라 **전환 판단 기준**.
- 세 단계는 독립 배포 가능. 1·2단계는 파일이 겹치지 않아 병렬 진행 가능. 3단계는 1·2단계 운용 결과를 보고 결정.
